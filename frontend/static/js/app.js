...
const TOKEN_EMBLEMS={
  ...
  NEAR:'N',APT:'A',ARB:'A',OP:'O',SUI:'S',FIL:'F',INJ:'I',ETC:'Ξ',
  AAVE:'A',MKR:'M',RUNE:'R',SEI:'S',TIA:'T',WIF:'W',PEPE:'P',SHIB:'S',
...
async function aiAsk(prefill=null){
  ...
  try{
    ...
    const d=await r.json();
    if(!d.success) throw new Error(d.error||'AI request failed — check OpenRouter key/model in Render');
...
const robotChatOpen=document.getElementById('robotChatOpen');
const robotAiState=document.getElementById('robotAiState');
if(robotChatOpen) robotChatOpen.addEventListener('click',()=>{
  aiPanel.classList.add('open');aiPanel.setAttribute('aria-hidden','false');aiCheckStatus();
});
...
async function refreshDiagnostics(){
  try{
    ...
    if(robotAiState){
      robotAiState.textContent=d.ai?.configured?'ONLINE':'KEY NEEDED';
      robotAiState.style.color=d.ai?.configured?'#43f3a4':'#ffd45d';
    }
    const ws=document.getElementById('deltaWsState');
    if(ws){
      ws.textContent=d.delta_ws_connected?'WS LIVE':'RECONNECTING';
      ws.className=d.delta_ws_connected?'ok':'';
    }
    const arm=document.getElementById('tradeArmStatus');
    if(document.getElementById('tradeEnableSwitch')) document.getElementById('tradeEnableSwitch').checked=!!d.trading_enabled;
    if(arm){
      arm.textContent=d.trading_enabled?'ON':'OFF';
      arm.style.color=d.trading_enabled?'#45f2a0':'#ffd056';
    }
  }catch(e){}
}
...
const tradeEnableSwitch=document.getElementById('tradeEnableSwitch');
if(tradeEnableSwitch) tradeEnableSwitch.addEventListener('change',async()=>{
  const requested=tradeEnableSwitch.checked;
  if(requested && !confirm('Enable LIVE trading? Orders can use real funds if Delta trading is enabled.')){
    tradeEnableSwitch.checked=false; return;
  }
  try{
    ...
    const d=await r.json();
    if(!d.success) throw new Error(d.error||'Trading toggle failed');
    tradeEnableSwitch.checked=!!d.enabled;
    refreshDiagnostics();
  }catch(e){
    tradeEnableSwitch.checked=false;
    alert(e.message||e);
  }
});

const tokenMasterBtn=document.getElementById('tokenMasterBtn');
if(tokenMasterBtn) tokenMasterBtn.addEventListener('click',async()=>{
  const d=await api('/api/tokens/master',{method:'POST'});
  tokenMasterBtn.textContent='TOKENS '+(d.enabled?'ON':'OFF');
});
document.querySelectorAll('[data-scan-group]').forEach(btn=>btn.addEventListener('click',async()=>{
